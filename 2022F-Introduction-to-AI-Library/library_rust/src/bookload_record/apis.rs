...
use crate::bookload_record::responses::BookTypeResponse;
...
pub async fn loan_info(rb: &State<Arc<Rbatis>>, userinfo: IsLogin) -> status::Accepted<content::Json<String>> {
    ...
    ans.insert("time", loan_info[0].loan_date.format("%Y年%m月%d日").to_string());
    let count_wrapper = rb
        .new_wrapper()
        .eq("book_id", book.id)
        .ne("user_id", &user.user_id)
        .group_by(&["user_id, id"])
        ;
...
pub async fn loan_type(rb: &State<Arc<Rbatis>>, userinfo: IsLogin) -> status::Accepted<Json<String>> {
    ...
    let types: Vec<Book> = get_types(rb, &(userinfo.0.user_id), &3usize).await.unwrap();
    ...
    let ans = ans.iter()
        .map(|x|BookTypeResponse{cnt: *x.1, name: x.0.to_string()})
        .collect::<Vec<BookTypeResponse>>();
    let mut result = HashMap::new();
    result.insert("list", ans);
    return status::Accepted(Some(Json(serde_json::to_string(&result).unwrap())))
}


#[post("/words")]
pub async fn words(rb: &State<Arc<Rbatis>>, userinfo: IsLogin) -> status::Accepted<Json<String>> {
    let mut types: Vec<Book> = get_words(rb, &(userinfo.0.user_id), &15usize)
        .await
        .unwrap();
    let mut ans: HashMap<&str, Vec<String>> = HashMap::new();

    let mut words = vec![];
    for mut i in &mut types {
        if words.len() > 20 {
            break;
        }
        if i.r#type.len() >= 1 {
            i.r#type.strip_prefix("['");
            i.r#type.strip_suffix("']");
            words.push(i.r#type.clone());
        }
        // if i.keyword.len() >= 1 {
        //     i.keyword.strip_prefix("['");
        //     i.keyword.strip_suffix("']");
        //     words.push(i.keyword.clone());
        // }
        // if i.writer_keyword.len() >= 1 {
        //     i.writer_keyword.strip_prefix("['");
        //     i.writer_keyword.strip_suffix("']");
